...
EM_JS(void, upload, (char const* accept_types, upload_handler callback, void* callback_data), {
  ...
  /// Accept-types are in the format ".png,.jpeg,.jpg" as per
  /// https://developer.mozilla.org/en-US/docs/Web/HTML/Attributes/accept Upload handler callback
  /// signature is:
  ///   void my_handler(std::string const filename, std::string const &mime_type, std::string_view
  ///   buffer, void *callback_data = nullptr);
  globalThis.open_file = function(e) {
    ...
    file_reader.addEventListener(
        "loadend", function() {
          ...
          const res = Module.ccall('load_file_return', 'number',
                                   [ 'string', 'string', 'number', 'number', 'number', 'number' ], [
                                     event.target.filename, event.target.mime_type,
                                     data_on_heap.byteOffset, uint8Arr.length, callback,
                                     callback_data
                                   ]);
...
EM_JS(void, download,
      (char const* filename, char const* mime_type, void const* buffer, size_t buffer_size), {
        /// Offer a buffer in memory as a file to download, specifying download filename and mime
        /// type
        var a = document.createElement('a');
        a.download = UTF8ToString(filename);
        a.href = URL.createObjectURL(
            new Blob([new Uint8Array(Module.HEAPU8.buffer, buffer, buffer_size)], {
              type:
...
EM_JS(void, ui_show_error_int, (), { showErrorModal(); });
...
EM_JS(void, open_int, (const char* name, const char* modes), {
  ...
  Module.writer.ready.then(() = > { console.log("zipstream: open, ready"); });
});
...
EM_ASYNC_JS(void, abort_int, (void), { Module.writer.abort(); });
